...
**バージョン**: 1.8.0（有料プランでのネイティブMCP機能拡張情報を追加）
**メンテナー**: HadayaLab
...
- **結論**: **無料でセルフホスト版で利用したい人向け、またはノード検索・テンプレート検索が必要な場合**
  - n8n Cloud: Personal Access Token取得には有料プラン（Starter以上、24€/mo）が必要
  - n8n Self-hosted: Personal Access Tokenを無料で取得可能
  - **ノード検索（543個）とテンプレート検索（2,700+）に特化**

**有料プランでの推奨**: n8nを有料プランにする場合、ネイティブMCPでほとんどのオペレーションが可能になるため、n8n-mcpパッケージは主にノード検索・テンプレート検索用途として使用することを推奨します。
...
**重要**: **n8nを有料プランにすると、ネイティブのn8n-MCPでほとんどのオペレーションができるようになります**。以下の制限は、無料プランでの制限です。

#### 1. ワークフローの作成（有料プランでは可能）
- **無料プランでの理由**: ネイティブMCPはワークフローの実行に特化
- **有料プラン**: ✅ ワークフローの作成が可能
- **無料プランでの代替方法**: n8n-mcpパッケージを使用するか、n8n Dashboardから手動作成

#### 2. ワークフローの更新（有料プランでは可能）
- **無料プランでの理由**: ワークフローの編集機能は提供されていない
- **有料プラン**: ✅ ワークフローの更新が可能
- **無料プランでの代替方法**: n8n-mcpパッケージを使用するか、n8n Dashboardから手動編集

#### 3. ワークフローの削除（有料プランでは可能）
- **無料プランでの理由**: ワークフロー削除機能は提供されていない
- **有料プラン**: ✅ ワークフローの削除が可能
- **無料プランでの代替方法**: n8n Dashboardから手動削除、またはREST APIを使用

#### 4. ノード検索（n8n-mcpパッケージを使用）
- **理由**: ノード検索機能は提供されていない（有料プランでも同様）
- **代替方法**: n8n-mcpパッケージの`list_node_templates`を使用

#### 5. テンプレート検索（n8n-mcpパッケージを使用）
- **理由**: テンプレート検索機能は提供されていない（有料プランでも同様）
- **代替方法**: n8n-mcpパッケージの`search_templates`を使用

#### 6. ワークフロー検証（n8n-mcpパッケージを使用）
- **理由**: ワークフロー検証機能は提供されていない（有料プランでも同様）
